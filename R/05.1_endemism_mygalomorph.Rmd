...
combinedShp_points <- terra::vect(species_points_clean)

## seems like phyloregion::points2comm() only needs original data format
...
comm.point <- phyloregion::points2comm(dat = myg_spiders_cleaned_points, res = 0.05, lon = "decimalLongitude", lat = "decimalLatitude", species = "species")
plot(comm.point$map)
...
comm_merged <- rbind(comm.poly_all_spp, comm.point_all_spp)
...
combinedShp_ex_points <- terra::vect(species_points_ex_clean)

## seems like phyloregion::points2comm() only needs original data format
myg_spiders_expert_points <- myg_spiders_expert |>
...
```{r}
# community matrix for species with alpha hulls (at least 3 records)
comm.poly_ex <- phyloregion::polys2comm(dat = combinedShp_ex, trace=1, res = 0.05)
str(comm.poly_ex)
plot(comm.poly_ex$map)

# community matrix for species without alpha hulls (<3 records)
comm.point_ex <- phyloregion::points2comm(dat = myg_spiders_expert_points, res = 0.05, lon = "decimalLongitude", lat = "decimalLatitude", species = "species")
plot(comm.point_ex$map)

# combining
all_species_ex <- union(colnames(comm.poly_ex$comm_dat), colnames(comm.point_ex$comm_dat))

missing_in_poly_ex <- setdiff(all_species, colnames(comm.poly_ex$comm_dat))
...
missing_in_point_ex <- setdiff(all_species, colnames(comm.point_ex$comm_dat))
...
comm_merged_ex <- rbind(comm.poly_all_spp_ex, comm.point_all_spp_ex)
```
