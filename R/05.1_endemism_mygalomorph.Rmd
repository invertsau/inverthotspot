...
species_polys_points <- readRDS(here("output/spatial", paste0("Spatial_alpha_hulls_sf_class_", file_name, ".rds")))

species_polys_points_expert <- readRDS(here("output/spatial", paste0("Spatial_alpha_hulls_sf_class_expert_", file_name, ".rds")))

species_polys_points_hum_obs <- readRDS(here("output/spatial", paste0("Spatial_alpha_hulls_sf_class_human_obs_", file_name, ".rds")))
```
...
names(decay_std) <- "population density influence"
names(sampling_rate_raster_ea_resample_std) <- "log sampling rate (sampbias)"
names(tot_samp_bias_norm) <- "final sampling bias"

plot(c(decay_std, sampling_rate_raster_ea_resample_std, tot_samp_bias_norm))

saveRDS(decay_std, "output/sampling_bias/pop_density_influence.rds")
saveRDS(sampling_rate_raster_ea_resample_std, "output/sampling_bias/sampbias_sampling_rate.rds")
saveRDS(tot_samp_bias_norm, "output/sampling_bias/final_sampling_bias.rds")
...
writeRaster(merged_geo, here("data/protected_areas/bias_corrected_hotspots_spatraster.tif"), datatype = "FLT8S", overwrite=TRUE)
...
# exporting for results section plotting
write_csv(merged_geo_df, here("data/dataframes_for_plots/full_dataset_hotspots.csv"))
```
...
#exporting for protected area analysis
writeRaster(merged_sr_geo, here("data/protected_areas/sr_bias_corrected_hotspots_spatraster.tif"), datatype = "FLT8S", overwrite=TRUE)
...
# exporting for results section plotting
write_csv(merged_geo_sr_df, here("data/dataframes_for_plots/full_dataset_hotspots_sr.csv"))
```
...
merged_ex <- c(WE_ex_samp_bias, 
               # CWE_ex_samp_bias, 
               richness_ex_samp_bias)
names(merged_ex) <- c("WE_ex_sampbias_corrected", 
                      # "CWE_ex_sampbias_corrected", 
                      "richness_ex_sampbias_corrected")

# projecting back to WGS84 for visualisation
merged_ex_geo <- project(merged_ex, "EPSG:4326")

#exporting for protected area analysis
writeRaster(merged_ex_geo, here("data/protected_areas/ex_bias_corrected_hotspots_spatraster.tif"), datatype = "FLT8S", overwrite=TRUE)
...
# exporting for results section plotting
write_csv(merged_geo_ex_df, here("data/dataframes_for_plots/expert_hotspots.csv"))
```
...
# merging alpha hull- and point-based maps, ensuring that richness is summed in overlapping cells
comm_overlapping_sr_ex <- merge(comm.poly_sr_ex$map, comm.point_ex$map, by = "grids", all.x = TRUE)
...
non_overlapping_sr_ex <- comm.point_ex$map[which(!values(comm.point_ex$map)[1]$grids %in% values(comm.poly_sr_ex$map)[1]$grids),]
...
#exporting for protected area analysis
writeRaster(merged_sr_ex_geo, here("data/protected_areas/sr_ex_bias_corrected_hotspots_spatraster.tif"), datatype = "FLT8S", overwrite=TRUE)
...
# exporting for results section plotting
write_csv(merged_geo_sr_ex_df, here("data/dataframes_for_plots/expert_hotspots_sr.csv"))
```
...
#exporting for protected area analysis
writeRaster(merged_hum_obs_geo, here("data/protected_areas/hum_obs_bias_corrected_hotspots_spatraster.tif"), datatype = "FLT8S", overwrite=TRUE)
...
# exporting for results section plotting
write_csv(merged_geo_hum_obs_df, here("data/dataframes_for_plots/hum_obs_hotspots.csv"))
```
...
```{r}
# getting species with range size of 10000 m2 or less (<401 grid cells)
short_range_spp_hum_obs <- colnames(comm_merged_hum_obs[,which(colSums(comm_merged_hum_obs)<401)])

# Extract POLYGONs from GEOMETRYCOLLECTIONs
species_polys_clean_sr_hum_obs <- species_polys_hum_obs_clean |> 
  filter(species %in% short_range_spp_hum_obs)
  
# Combine into one large spatial dataframe
combinedShp_sr_hum_obs <- terra::vect(species_polys_clean_sr_hum_obs)

#projecting to Albers Equal Area projection (EPSG:9473 (GDA2020 / Australian Albers))
combinedShp_equalarea_sr_hum_obs <- project(combinedShp_sr_hum_obs, "EPSG:9473") 
...
```{r}
# community matrix for species with alpha hulls (at least 3 records)
comm.poly_sr_hum_obs <- phyloregion::polys2comm(dat = combinedShp_equalarea_sr_hum_obs, trace=1, res = 5000)

## for species without alpha hulls
## seems like phyloregion::polys2comm() also works for point data if in spatvector format
myg_spiders_cleaned_points_hum_obs <- myg_spiders_cleaned |>
  filter(scientificName %in% species_points_clean$species) |>
  dplyr::select(species, decimalLongitude, decimalLatitude) |>
  sf::st_as_sf(coords = c("decimalLongitude", "decimalLatitude"), crs = "EPSG:4326") |>
  sf::st_transform(crs = "EPSG:9473") #projecting to Albers Equal Area projection (EPSG:9473 (GDA2020 / Australian Albers))
...
```{r}
comm_merged_short_range_spp_hum_obs <- comm_merged_hum_obs[,which(colSums(comm_merged_hum_obs)<401)]

Endm.mygalomorph_sr_hum_obs <- phyloregion::weighted_endemism(comm_merged_short_range_spp_hum_obs)

# merging alpha hull- and point-based maps, ensuring that richness is summed in overlapping cells
comm_overlapping_sr_hum_obs <- merge(comm.poly_sr_hum_obs$map, comm.point_hum_obs$map, by = "grids", all.x = TRUE)
comm_overlapping_sr_hum_obs$richness <- ifelse(is.na(comm_overlapping_sr_hum_obs$richness.x), 0, comm_overlapping_sr_hum_obs$richness.x) + ifelse(is.na(comm_overlapping_sr_hum_obs$richness.y), 0, comm_overlapping_sr_hum_obs$richness.y)

non_overlapping_sr_hum_obs <- comm.point_hum_obs$map[which(!values(comm.point_hum_obs$map)[1]$grids %in% values(comm.poly_sr_hum_obs$map)[1]$grids),]

comm_merged_map_sr_hum_obs <- rbind(comm_overlapping_sr_hum_obs, non_overlapping_sr_hum_obs)

## Join endemism results back to spatial community data matrix
m1.poly_merged_sr_hum_obs <- merge(comm_merged_map_sr_hum_obs, data.frame(grids=names(Endm.mygalomorph_sr_hum_obs), WE=Endm.mygalomorph_sr_hum_obs), by="grids")

# Drop NA in WE
m1.poly_sr_hum_obs <- m1.poly_merged_sr_hum_obs[!is.na(m1.poly_merged_sr_hum_obs$WE),]
...
```{r}
m1.poly_sr_hum_obs$corrected_endemism <- m1.poly_sr_hum_obs$WE/m1.poly_sr_hum_obs$richness
```

### Assigning hotspots based on percentile

```{r}
m1.poly_sr_hum_obs$rich_perc <- m1.poly_sr_hum_obs$richness > quantile(m1.poly_sr_hum_obs$richness, probs = 0.95)
m1.poly_sr_hum_obs$WE_perc <- m1.poly_sr_hum_obs$WE > quantile(m1.poly_sr_hum_obs$WE, probs = 0.95)
m1.poly_sr_hum_obs$CWE_perc <- m1.poly_sr_hum_obs$corrected_endemism > quantile(m1.poly_sr_hum_obs$corrected_endemism, probs = 0.95)
```
...
```{r}
crs(m1.poly_sr_hum_obs) <- "EPSG:9473"

# projecting back to WGS84 for visualisation
m1_sf_sr_hum_obs <- 
  sf::st_as_sf(m1.poly_sr_hum_obs) |> 
  sf::st_transform(4326)

ggplot() +
  geom_sf(data = ozmap_states, fill = "white") + 
  geom_sf(data = m1_sf_sr_hum_obs, aes(fill = WE), col = NA) + 
  geom_sf(data = subset(m1_sf_sr_hum_obs, WE_perc), fill = "red", col = NA) +
  geom_sf(data = ozmap_states, fill = NA, colour = "black") + 
  coord_sf(xlim = c(110, 155),
          ylim = c(-10, -45)) +
  viridis::scale_fill_viridis(option = "G",direction = -1#, values = c(0,0.01,0.1,1)
...
ggplot() +
  geom_sf(data = ozmap_states, fill = "white") + 
  geom_sf(data = m1_sf_sr_hum_obs, aes(fill = corrected_endemism), col = NA) + 
  geom_sf(data = subset(m1_sf_sr_hum_obs, CWE_perc), fill = "red", col = NA) +
  geom_sf(data = ozmap_states, fill = NA, colour = "black") + 
  coord_sf(xlim = c(110, 155), 
           ylim = c(-10, -45)) + 
  viridis::scale_fill_viridis(option = "G",direction = -1) + 
...
ggplot() +
  geom_sf(data = ozmap_states, fill = "white") + 
  geom_sf(data = m1_sf_sr_hum_obs, aes(fill = richness), col = NA) + 
  geom_sf(data = subset(m1_sf_sr_hum_obs, rich_perc), fill = "red", col = NA) +
  geom_sf(data = ozmap_states, fill = NA, colour = "black") + 
  coord_sf(xlim = c(110, 155), 
           ylim = c(-10, -45)) + 
  viridis::scale_fill_viridis(option = "G",direction = -1) + 
...
```{r}
# testing as raster
m1.poly_sr_hum_obs_rast <- rast(m1.poly_sr_hum_obs, res = 5000)

# transferring from m1.poly attributes into a SpatRaster with multiple layers
m1.poly_sr_hum_obs_raster <- rast(lapply(names(m1.poly_sr_hum_obs)[c(6,8,9)], function(att) {
  rasterize(m1.poly_sr_hum_obs, m1.poly_sr_hum_obs_rast, field = att)
}))

# setting crs
crs(m1.poly_sr_hum_obs_raster) <- crs(sampling_rate_raster_ea)

m1.poly_sr_hum_obs_raster <- resample(m1.poly_sr_hum_obs_raster, tot_samp_bias_norm)


# correcting biodiversity metrics based on (log+1) estimated sampling rate (dividing by sampling rate)
WE_sr_hum_obs_samp_bias <- terra::app(x = c(m1.poly_sr_hum_obs_raster$WE, tot_samp_bias_norm), 
                           fun = function(x){return(x[1]/x[2])})
CWE_sr_hum_obs_samp_bias <- terra::app(x = c(m1.poly_sr_hum_obs_raster$corrected_endemism, tot_samp_bias_norm), 
                                 fun = function(x){return(x[1]/x[2])})
richness_sr_hum_obs_samp_bias <- terra::app(x = c(m1.poly_sr_hum_obs_raster$richness, tot_samp_bias_norm), 
                                 fun = function(x){return(x[1]/x[2])})

merged_sr_hum_obs <- c(WE_sr_hum_obs_samp_bias, CWE_sr_hum_obs_samp_bias, richness_sr_hum_obs_samp_bias)
names(merged_sr_hum_obs) <- c("WE_sr_hum_obs_sampbias_corrected", "CWE_sr_hum_obs_sampbias_corrected", "richness_sr_hum_obs_sampbias_corrected")

# projecting back to WGS84 for visualisation
merged_sr_hum_obs_geo <- project(merged_sr_hum_obs, "EPSG:4326")

#exporting for protected area analysis
writeRaster(merged_sr_hum_obs_geo, here("data/protected_areas/sr_hum_obs_bias_corrected_hotspots_spatraster.tif"), datatype = "FLT8S", overwrite=TRUE)

# converting to data frame for plotting
merged_geo_sr_hum_obs_df <- as.data.frame(merged_sr_hum_obs_geo, xy = TRUE, na.rm = TRUE)

# Assigning hotspots based on percentile
merged_geo_sr_hum_obs_df$WE_perc <- merged_geo_sr_hum_obs_df$WE_sr_hum_obs_sampbias_corrected > quantile(merged_geo_sr_hum_obs_df$WE_sr_hum_obs_sampbias_corrected, probs = 0.95)
merged_geo_sr_hum_obs_df$CWE_perc <- merged_geo_sr_hum_obs_df$CWE_sr_hum_obs_sampbias_corrected > quantile(merged_geo_sr_hum_obs_df$CWE_sr_hum_obs_sampbias_corrected, probs = 0.95)
merged_geo_sr_hum_obs_df$rich_perc <- merged_geo_sr_hum_obs_df$richness_sr_hum_obs_sampbias_corrected > quantile(merged_geo_sr_hum_obs_df$richness_sr_hum_obs_sampbias_corrected, probs = 0.95)

# exporting for results section plotting
write_csv(merged_geo_sr_hum_obs_df, here("data/dataframes_for_plots/hum_obs_hotspots_sr.csv"))
```
...
ggplot() +
  geom_sf(data = ozmap_states, fill = "white") + 
  geom_tile(data = merged_geo_sr_hum_obs_df, aes(x=x, y=y, fill = WE_sr_hum_obs_sampbias_corrected), col = NA) + 
  geom_tile(data = subset(merged_geo_sr_hum_obs_df, WE_perc), aes(x=x, y=y), fill = "red", col = NA) +
  geom_sf(data = ozmap_states, fill = NA, colour = "black") + 
  coord_sf(xlim = c(110, 155),
          ylim = c(-10, -45)) +
  viridis::scale_fill_viridis(option = "G",direction = -1#, values = c(0,0.01,0.1,1)
...
ggplot() +
  geom_sf(data = ozmap_states, fill = "white") + 
  geom_tile(data = merged_geo_sr_hum_obs_df, aes(x=x, y=y, fill = CWE_sr_hum_obs_sampbias_corrected), col = NA) + 
  geom_tile(data = subset(merged_geo_sr_hum_obs_df, CWE_perc), aes(x=x, y=y), fill = "red", col = NA) +
  geom_sf(data = ozmap_states, fill = NA, colour = "black") + 
  coord_sf(xlim = c(110, 155),
          ylim = c(-10, -45)) +
  viridis::scale_fill_viridis(option = "G",direction = -1#, values = c(0,0.01,0.1,1)
...
ggplot() +
  geom_sf(data = ozmap_states, fill = "white") + 
  geom_tile(data = merged_geo_sr_hum_obs_df, aes(x=x, y=y, fill = richness_sr_hum_obs_sampbias_corrected), col = NA) + 
  geom_tile(data = subset(merged_geo_sr_hum_obs_df, rich_perc), aes(x=x, y=y), fill = "red", col = NA) +
  geom_sf(data = ozmap_states, fill = NA, colour = "black") + 
  coord_sf(xlim = c(110, 155),
          ylim = c(-10, -45)) +
  viridis::scale_fill_viridis(option = "G",direction = -1#, values = c(0,0.01,0.1,1)
