...
myg_spiders <- myg_spiders_all |> 
  dplyr::select(decimalLatitude:collectionCode, countryConservation, stateConservation) |> 
  collect()
...
```{r}
conservation_sig <- myg_spiders |> filter(!is.na(countryConservation) | !is.na(stateConservation))
tabyl(conservation_sig$species)
```
...
fuzzy_taxa <- myg_spiders_all |>
  filter(TAXON_MATCH_FUZZY == TRUE) |>
  dplyr::select(species, scientificName, raw_scientificName) |>
  collect() |>
  unique() |>
  print(n = Inf)
```
