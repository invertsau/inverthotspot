...
Below shows the total counts of the lowest taxonomic rank provided for each record.
...
taxonRank = unranked for Selenocosmia crassipes as this taxon represents a cryptic species complex with unresolved phylogeny.
...
MACHINE OBSERVATION: An occurrence record describing an observation made by a machine.
...
- COORDINATE_ROUNDED - the original coordinates were rounded to six decimals (~ 1 m precision) to simplify processing.
...
TAXON_EXCLUDED - is flagged when the supplied scientific name (raw_scientificName) matches a taxon that is not known to occur in Australia. Could the expert review the following list and assess whether raw_scientificName is likely to be an incorrect identification and therefore remain excluded.
```{r, echo=FALSE}
# List of species (adjusted name) and raw names (names given in observation)
...
TAXON_QUESTION_SPECIES - is flagged when the supplied scientific name (raw_scientificName) contains a question mark, indicating an uncertain identification. 
```{r, echo=FALSE}
# List of species (adjusted name) and raw names (names given in observation)
