...
Of the **`r unique_species`** unique **`r focus_taxon_short`** species in the ALA dataset, **`r AFD_matched`** matched those listed in the AFD. These **`r AFD_unmatched`** species did not match:
**`r unmatched_ala_sp`**.
...
**`r unmatched_ala_sp[issynonym]`**.

**`r unmatched_ala_sp[issynonym]`** is a synonym of 
**`r arachnids[which(word(arachnids$SYNONYMS, 1, 2) == unmatched_ala_sp[issynonym]),18]`** 
**`r arachnids[which(word(arachnids$SYNONYMS, 1, 2) == unmatched_ala_sp[issynonym]),20]`**.
...
no_Introduced_WSC <- which(myg_species$scientificName %in% introduced_taxa_wfullname$scientific_name)
```

There are **`no`r no_Introduced_WSC`** introduced species of spiders in the ALA data according to the WSC data.
