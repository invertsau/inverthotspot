...
# Deduplicating
deduplicated_myg_species <- myg_species_clean |> 
  filter(!is.na(decimalLatitude) & !is.na(decimalLongitude)) |> 
...
myg_cleaned_sp <- myg_species_clean |> 
  filter(taxonRank == "species")

# Summarise number of observations by taxon
deduplicated_myg_species |> 
  group_by(scientificName) |> 
  summarise(n = n()) |> 
  arrange(-n)

# Which ones have less than or equal 3 
less_than_3_obs <- deduplicated_myg_species |> 
  group_by(scientificName) |> 
  summarise(n = n()) |> 
  filter(n <= 3) |> 
  pull(scientificName)

myg_sp_enoughobs <- deduplicated_myg_species |> 
