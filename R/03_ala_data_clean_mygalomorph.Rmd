...
```{r}
myg_species_aus <- myg_species |> filter(decimalLatitude != "NA") #remove NA coordinates

summary(myg_species_aus$decimalLongitude) #NAs gone

pacman::p_load(ozmaps, sf, CoordinateCleaner)


#clean using Coordinate Cleaner

myg_species_aus <- clean_coordinates(x = myg_species)
...
myg_species_sf <- myg_species_aus |> 
  select(scientificName, decimalLongitude, decimalLatitude) |> 
  st_as_sf(coords = c("decimalLongitude", "decimalLatitude"), 
           crs = 4326)

base_map  + 
  geom_sf(data = myg_species_sf, alpha = 0.5) + 
  theme_minimal()
