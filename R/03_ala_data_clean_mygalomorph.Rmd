...
myg_spiders <- myg_spiders_all |> 
  dplyr::select(recordID:outlierLayer) |> 
  collect()
...
```{r, echo=F}
AFD_synonyms <- afd_myg |> filter(!is.na(SPECIES),
                                  NAME_TYPE == "Synonym")
AFD_synonyms <- word(AFD_synonyms$SCIENTIFIC_NAME, 1,2, sep = " ")
...
These will be verified by experts to determine how to best handle them. - After consulting experts and checking the [World Spider Catalogue (WSC)](https://wsc.nmbe.ch/) it was determined that *Selenocosmia crassipes* is the correct name, and all *Phlogius crassipes* records were changed in the ALA dataset.
...
myg_species <- myg_spiders_no_unmatched$species |> unique()


job::job({
  output_df <- purrr::map(myg_species,
...
wormsOutput |> dplyr::select(scientificname, search_term, match_type)
names(wormsOutput)
...
myg_spiders_no_unmatched <- myg_spiders_no_unmatched |> filter(country == "Australia")
...
map_area <- borders(col = "grey80", 
          xlim = range(myg_spiders_no_unmatched$decimalLongitude), 
          ylim = range(myg_spiders_no_unmatched$decimalLatitude))
ggplot() +
  coord_fixed() + 
  map_area + 
...
`.equ` = Equal longitude and latitude `.sea` = XXX
...
The `r summary(flagged_alltests)[".sea"] |> as.vector()` records excluded due to being in the ocean. (`r (( summary(flagged_alltests)[".sea"] |> as.vector() / nrow(myg_spiders)) *100 )|> round(2)`%)
...
#checking for records with lat, long, date, and species duplicated
lat_long_species_dups <- duplicated(myg_spiders_clean_coords[,c(4:6,14)]) |
  duplicated(myg_spiders_clean_coords[,c(4:6,14)], fromLast = TRUE)

tabyl(lat_long_species_dups) # Number of records with lat, long, date, and species duplicated
...
```{r}
expert_records <- deduplicated_myg_spiders |> 
...
write_parquet(deduplicated_myg_spiders, here(paste0("output/data/cleaned_", file_name)))
