...
<!-- Export as CSV -->
```{r}
write.csv(myg_species_clean, 
          paste0("output/Cleaned_Data_Mygalomorphae_", Sys.Date(), ".csv"))
```
...
#myg_spiders$speciesconcatenate <- myg_spiders |> paste(scientificName, decimalLatitude, decimalLongitude)
???

myg_spiders$speciesconcatenate <- with(myg_spiders, paste(scientificName, decimalLatitude, decimalLongitude))

head(myg_spiders$speciesconcatenate)

Mygalomorphae_clean <- myg_spiders |> 
  filter(!duplicated(speciesconcatenate))

nrow(Mygalomorphae_clean)
nrow(myg_spiders) - nrow(Mygalomorphae_clean)
