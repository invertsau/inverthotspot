...
```{r, echo=F}
flagged <- clean_coordinates(x = myg_species, 
                                     "decimalLongitude",
                                     "decimalLatitude")
```
...
ggplot() +
  ...
  geom_point(data = myg_species,
             ...
             colour = "lightgreen",
             size = 0.5) +
  geom_point(data = subset(flagged, .summary==FALSE),
             aes(x = decimalLongitude, y = decimalLatitude),
             colour = "darkred",
             size = 0.5) +
  theme_bw()
...
What do the codes mean:
  .val = coordinate validity: non-numeric or missing coordinates and coordinates exceeding the global extent
  .equ = equal: tests for equal absolute longitude and latitude
  .zer = zeros: tests for plain zeros, equal latitude and longitude and a radius around the point 0/0
  .cap = capitals: tests a radius around adm-0 capitals
  .cen = centroids: tests a radius around country centroids
  .sea = seas: tests if coordinates fall into the ocean
  .otl = outliers: tests each species for outlier records
  .gbf = gbif: tests a one-degree radius around the GBIF headquarters in Copenhagen, Denmark
  .inst = institutions: tests a radius around known biodiversity institutions from instiutions
  .summary = False if any test flagged the respective coordinate (clean coordinates are "True")
...
```{r, echo=F}
myg_species_clean <- myg_species[which(flagged$.summary),]
```

### Report how many records were removed under each Coordinate Cleaner test

```{r}
#how many flagged by summary?
nrow(flagged) #total records
nrow(subset(flagged, .summary==TRUE)) #clean records
nrow(subset(flagged, .summary==FALSE)) #flagged records
#was that the number removed?
nrow(myg_species_clean) == nrow(subset(flagged, .summary==TRUE)) #yes.

#How many were flagged with each test
summary(flagged)
sum(flagged$.summary == FALSE)
sum(flagged$.val == FALSE)
sum(flagged$.equ == FALSE)
sum(flagged$.zer == FALSE)
sum(flagged$.cap == FALSE)
sum(flagged$.cen == FALSE)
sum(flagged$.sea == FALSE)
sum(flagged$.otl == FALSE)
sum(flagged$.gbf == FALSE)
sum(flagged$.inst == FALSE)
```
