...
```{r}
merged_geo <- readRDS(merged_geo, here("data/protected_areas/bias_corrected_hotspots_spatraster.rds"))
...
overlap_richness <- !is.na(aus_pa_raster) & merged_geo[["rich_perc"]]
overlap_richness_df <- as.data.frame(overlap_richness, xy = TRUE, na.rm = TRUE)

base_map +
  # geom_tile(data = aus_pa_raster_df,
  #           aes(x=x, y=y)) +
  geom_tile(data = subset(overlap_richness_df, layer),
            aes(x=x, y=y), fill = "red", col = NA) +
          # aes(geometry = geometry),
          # colour = "#609966",
          # fill = "lightgreen") +
  # geom_sf(data = points,
