...
# richness 95th percentile
q95 <- global(merged_geo[["richness_sampbias_corrected"]], quantile, probs = 0.95, na.rm = TRUE)[1, 1]
merged_geo$rich_perc <- merged_geo[["richness_sampbias_corrected"]] > q95
...
# getting cells where richness hotspots overlap with PA's
overlap_richness <- (!is.na(aus_pa_raster)) & (merged_geo[["rich_perc"]] > 0)
...
overlap_richness_df <- as.data.frame(overlap_richness, xy = TRUE, na.rm = TRUE)

freq(overlap_richness)[2,3]/freq(merged_geo$rich_perc)[2,3]*100 # percentage of richness hotspots within PA's
...
ggplot() +
  ...
  geom_tile(data = subset(overlap_richness_df, layer), aes(x=x, y=y), fill = "darkgreen", col = NA) +
