...
myg_spiders <- myg_spiders_all |> 
  select(recordID:collectionCode) |> 
  filter(basisOfRecord %in% c("PRESERVED_SPECIMEN", "MATERIAL_SAMPLE")) |>
  collect()

# Subset data for assertions
...
There are **`r total_myg` species** in total with **`r nrow(myg_spiders)`** of records
...
For this analysis we need at least 3 records to create alpha-hulls. 
...
`r nrow(myg_spp)` species (`r round((nrow(myg_spp)/total_myg)*100, digits = 1)`%) have 3 or more records. Number of records = `r nrow(myg_spiders |> group_by(species) |> filter(n() > 2) )`
...
<!-- #### Top species by family  -->

```{r, results = 'asis', echo = FALSE}
species_in_family <- function(myg_spiders, family){
  myg_spiders |>
...
cat('#### Top species by family {.tabset}   \n Most recorded species in each family (up to 10 species). Select tabs to see plot for corresponding family. \n\n')
  myg_fam_ls  <- myg_spiders |> 
      split(myg_spiders$family)
    
  purrr::walk2(.x = myg_fam_ls,
               .y = unique(myg_spiders$family),
               ~{
                 # create tabset for each group 
                 cat('#####',.y,'   \n')
                 p <- species_in_family(.x, family = .y)
                 print(p)
                 cat('\n\n')
               })
```
...
myg_spiders |>
  filter(!basisOfRecord %in% c("OCCURRENCE", "OBSERVATION")) |>
  tabyl(basisOfRecord) |>
  mutate(percent = percent*100) |>
  arrange(desc(n)) |>
