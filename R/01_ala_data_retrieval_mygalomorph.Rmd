...
fields_we_want <- c(
  ...
  "stateProvince", "locality", "coordinatePrecision",  "coordinateUncertaintyInMeters", 
...
# Fix strange scientificName cols
mygalomorphae_occurrences_nm_fixed <- mygalomorphae_occurrences |> 
  rename(scientificName = scientificName...4,
         raw_scientificName = scientificName...17)
...
```{r}
mygalomorphae_occurrences_nm_fixed |> names()
mygalomorphae_occurrences_nm_fixed |> nrow()

mygalomorphae_assertions |> names()
mygalomorphae_assertions |> nrow()

joined_data <- left_join(mygalomorphae_occurrences_nm_fixed, mygalomorphae_assertions, by = "recordID")
```
...
```{r}
write_parquet(joined_data, paste0("data/galah/Mygalomorphae_withassertions_", Sys.Date(), "_ALA.parquet"))
```
