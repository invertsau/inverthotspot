...
pacman::p_load(galah, arrow, here, tidyverse, janitor, ozmaps, sf, ggplot2, wesanderson, dplyr, DT, sampbias, viridis, rnaturalearthdata, remotes, terra, occAssess, raster)

# require(devtools)
# #gitcreds::gitcreds_delete()
# install_github(repo = "azizka/sampbias")
# install_github(repo = "robboyd/occAssess")
...
myg_spiders_spatial_bias <- myg_spiders_cleaned |> 
...
aus_raster <- raster(st_transform(aus,"EPSG:9473"))
...
rasterlayer <- raster(sampling_rate_raster_ea)
```

## Broadly quantifying spatial bias
### full dataset

```{r}
myg_spiders_spat_bias <- myg_spiders_all |> 
  mutate(year = as.numeric(str_sub(eventDate,1,4))) |>
  dplyr::select(species, decimalLongitude, decimalLatitude, coordinateUncertaintyInMeters, year, taxonRank) |>
  as.data.frame() |>
  sf::st_as_sf(coords = c("decimalLongitude", "decimalLatitude"), crs = "EPSG:4326") |>
  sf::st_transform(crs = "EPSG:9473") #projmyg_spiders_spat_biasecting to Albers Equal Area projection (EPSG:9473 (GDA2020 / Australian Albers))

myg_spiders_spat_bias_df <- as.data.frame(st_coordinates(myg_spiders_spat_bias)) |>
  mutate(species = myg_spiders_spat_bias$species,
         spatialUncertainty = myg_spiders_spat_bias$coordinateUncertaintyInMeters,
         year = myg_spiders_spat_bias$year,
         taxonRank = myg_spiders_spat_bias$taxonRank)

spat_bias <- assessSpatialBias(myg_spiders_spat_bias_df, species = "species", x = "X", y = "Y", spatialUncertainty = "spatialUncertainty", year = "year", identifier = "taxonRank", periods = list(min(myg_spiders_spat_bias_df$year, na.rm = TRUE):max(myg_spiders_spat_bias_df$year, na.rm = TRUE)), mask = rasterlayer)
```

### expert only data
```{r}
myg_spiders_expert <- myg_spiders_all |>
  filter(!basisOfRecord == "HUMAN_OBSERVATION")

myg_spiders_ex_spat_bias <- myg_spiders_expert |> 
  mutate(year = as.numeric(str_sub(eventDate,1,4))) |>
  dplyr::select(species, decimalLongitude, decimalLatitude, coordinateUncertaintyInMeters, year, taxonRank) |>
  as.data.frame() |>
  sf::st_as_sf(coords = c("decimalLongitude", "decimalLatitude"), crs = "EPSG:4326") |>
...
myg_spiders_ex_spat_bias_df <- as.data.frame(st_coordinates(myg_spiders_ex_spat_bias)) |>
  mutate(species = myg_spiders_ex_spat_bias$species,
         spatialUncertainty = myg_spiders_ex_spat_bias$coordinateUncertaintyInMeters,
         year = myg_spiders_ex_spat_bias$year,
         taxonRank = myg_spiders_ex_spat_bias$taxonRank)

spat_bias_ex <- assessSpatialBias(myg_spiders_ex_spat_bias_df, species = "species", x = "X", y = "Y", spatialUncertainty = "spatialUncertainty", year = "year", identifier = "taxonRank", periods = list(min(myg_spiders_ex_spat_bias_df$year, na.rm = TRUE):max(myg_spiders_ex_spat_bias_df$year, na.rm = TRUE)), mask = rasterlayer)
```

### human observations only
```{r}
myg_spiders_hum_obs <- myg_spiders_all |>
  filter(basisOfRecord == "HUMAN_OBSERVATION")

myg_spiders_hum_obs_spat_bias <- myg_spiders_hum_obs |> 
  mutate(year = as.numeric(str_sub(eventDate,1,4))) |>
  dplyr::select(species, decimalLongitude, decimalLatitude, coordinateUncertaintyInMeters, year, taxonRank) |>
  as.data.frame() |>
  sf::st_as_sf(coords = c("decimalLongitude", "decimalLatitude"), crs = "EPSG:4326") |>
...
myg_spiders_hum_obs_spat_bias_df <- as.data.frame(st_coordinates(myg_spiders_hum_obs_spat_bias)) |>
  mutate(species = myg_spiders_hum_obs_spat_bias$species,
         spatialUncertainty = myg_spiders_hum_obs_spat_bias$coordinateUncertaintyInMeters,
         year = myg_spiders_hum_obs_spat_bias$year,
         taxonRank = myg_spiders_hum_obs_spat_bias$taxonRank)

spat_bias_hum_obs <- assessSpatialBias(myg_spiders_hum_obs_spat_bias_df, species = "species", x = "X", y = "Y", spatialUncertainty = "spatialUncertainty", year = "year", identifier = "taxonRank", periods = list(min(myg_spiders_hum_obs_spat_bias_df$year, na.rm = TRUE):max(myg_spiders_hum_obs_spat_bias_df$year, na.rm = TRUE)), mask = rasterlayer)

```

## Population density
data downloaded from the Australian Bereau of Statistics (Australian Bureau of Statistics. (2023-24). Regional population. ABS. https://www.abs.gov.au/statistics/people/population/regional-population/2023-24.)
