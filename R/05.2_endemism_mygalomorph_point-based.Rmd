...
Local Moran's I - to identify clusters
```{r}
RI_ex_local <- localmoran(m1.point_ex$WE, lw, alternative = "two.sided")
...
m1_sf_ex$lmp <- RI_ex_local[, "Pr(z != E(Ii))"]

p1 <- tm_shape(m1_sf_ex) +
  tm_polygons(col = "WE", title = "WE", style = "quantile") +
  tm_layout(legend.outside = TRUE)

p2 <- tm_shape(m1_sf_ex) +
  tm_polygons(col = "lmI", title = "Local Moran's I",
              style = "quantile") +
  tm_layout(legend.outside = TRUE)

p3 <- tm_shape(m1_sf_ex) +
  tm_polygons(col = "lmZ", title = "Z-score",
              breaks = c(-Inf, 1.65, Inf)) +
  tm_layout(legend.outside = TRUE)

p4 <- tm_shape(m1_sf_ex) +
...
mp <- moran.plot(as.vector(scale(m1_sf_ex$richness)), lw)
...
# high-high
m1_sf_ex[(mp$x >= 0 & mp$wx >= 0) & (!is.na(m1_sf_ex$lmp) & m1_sf_ex$lmp <= 0.05), "quadrant"]<- 1
# low-low
m1_sf_ex[(mp$x <= 0 & mp$wx <= 0) & (!is.na(m1_sf_ex$lmp) & m1_sf_ex$lmp <= 0.05), "quadrant"]<- 2
# high-low
m1_sf_ex[(mp$x >= 0 & mp$wx <= 0) & (!is.na(m1_sf_ex$lmp) & m1_sf_ex$lmp <= 0.05), "quadrant"]<- 3
# low-high
m1_sf_ex[(mp$x <= 0 & mp$wx >= 0) & (!is.na(m1_sf_ex$lmp) & m1_sf_ex$lmp <= 0.05), "quadrant"]<- 4
# non-significant
m1_sf_ex[(!is.na(m1_sf_ex$lmp) & m1_sf_ex$lmp > 0.05), "quadrant"] <- 5
