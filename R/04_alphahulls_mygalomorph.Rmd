...
polys <- all_myg_alpha_sf_points |>
  filter(st_geometry_type(conr_ahull_sf) == "GEOMETRYCOLLECTION")
points <- all_myg_alpha_sf_points |>
  filter(st_geometry_type(conr_ahull_sf) == "MULTIPOINT")


all_myg_alpha_sf_points <- all_myg_alpha_sf_points %>%
  mutate(
    geometry = st_sfc(
      map(conr_ahull_sf, ~{
        if (st_geometry_type(.x) == "MULTIPOINT") {
          st_geometrycollection(.x)   # wrap multipoint
        } else {
          .x                          # keep as-is
        }
      }),
