...
## Setting up
 pacman::p_load(arrow, here, tidyverse, janitor, ozmaps, sf, ggplot2,sp, purrr, ConR, dplyr, DT, zoomerjoin, gt, patchwork, tmap)
...
#Calculate weighted endemism
Endm.mygalomorph.point_ex <- phyloregion::weighted_endemism(comm.point$comm_dat)
...
RI_mp <- moran.plot(as.vector(scale(m1.point_sf$richness)), lw)
...
```{r}
# preserved specimen only dataset
RI_local_ex <- localmoran(m1.point_ex$richness, lw, alternative = "greater")
...
# moran's I scatterplot
RI_ex_mp <- moran.plot(as.vector(scale(m1.point_ex_sf$richness)), lw)
...
#| fig-width: 8
#| fig-cap: 
#|   - "Figure 5: Map showing estimated weighted endemism of Mygalmorphae spiders using point-based data. Left: All data from the cleaned dataset is used i.e contains citizen science data (basis of record == `HUMAN_OBSERVATION`) Right: Only expert data is used (basis of record == `PRESERVED_SPECIMEN` only)"
...
#| fig-width: 8
#| fig-cap: 
#|   - "Figure 6: Map showing estimated corrected weighted endemism of Mygalmorphae spiders using point-based data. Left: All data from the cleaned dataset is used i.e contains citizen science data (basis of record == `HUMAN_OBSERVATION`) Right: Only expert data is used (basis of record == `PRESERVED_SPECIMEN` only)"
...
Table 3. Spatial autocorrelation summary statistics for corrected weighted endemism for point-based data
