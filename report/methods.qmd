...
After some consideration, we decided to only remove those flagged as being in the vicinity of the capital city and biological institutions. We will consult the taxonomic experts regarding records flagged as being in the sea. 
...
The `r cc_summary[".cap"] |> as.vector()` records excluded due to being near a capitol vicinity. (`r (( cc_summary[".cap"] |> as.vector() / nrow(myg_spiders)) *100 ) |> round(2)`%)


The `r cc_summary[".inst"] |> as.vector()` records excluded due to being in the vicinity of biodiversity institutions. (`r (( cc_summary[".inst"] |> as.vector() / nrow(myg_spiders)) *100 )|> round(2)`%)
