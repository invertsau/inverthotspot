...
The taxonomic data was cleaned to remove:
...
Additionally, subspecies level identifications were reclassified to species level.

Here we install the necessary packages, load in the data, and subset the required columns.
...
myg_spiders <- myg_spiders_all |> select(recordID:outlierLayer) |> collect()
```

#### Taxonomic Errors

Records not identified to species level were removed.
...
We also used Australian Faunal Directory (AFD) data to ensure all species names in the ALA dataset are valid.
...
Here we open AFD data and check that our mygalomorphae data matches.
...
For records that do not match the AFD, we check if they are synonyms of listed species.
...
We used the World Spider Catalogue (WSC) and the Global Register of Invasive and Introduced Species (GRIIS) to exclude introduced spiders from the dataset.
...
We checked for subspecies by counting the number of words in the "species" field.
...
In this case, there were no subspecies to deal with.
...
This function cross matches our species list to the WoRMS database in the background.
