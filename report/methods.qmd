...
Calculation of Moran's I requires a list of neighbours for each spatial unit, along with weights denoting the spatial proximity between units. Neighbouring spatial units (cells for point-based analysis) were identified as those within a 200 km radius.
...
After testing for spatial autocorrelation across the whole study region (Australia), Local Moran's I [@anselin95] was used to assess the significance of clustering of similar values around each spatial unit, thereby allowing specific hotspots to be identified. To account for multiple comparisons, a significance level of P <= 0.0001 was used for local Moran’s I [@anselin95].
